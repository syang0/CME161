/**
 * This script takes the processed ramcloud dataset and puts it into cross filter
 * to allow a user to explore what makes rpcs slow.
 *
...
d3.json("/ramcloudData", function(ramcloud_dataset){
  ...
  // Extra hack to make the super long opcode's fit in in the graph.
  ...
  var typeChart = dc.barChart("#type_chart")
    .width(DWIDTH)
    .height(DHEIGHT)
    .dimension(dim_type)
    .group(typeSum)
    .centerBar(true)
    .x(d3.scale.ordinal().domain(["req", "res"]))
    .xUnits(dc.units.ordinal);
